...
# (IMERG) remapeado para a grade do MPAS, e a simulação do modelo
# MONAN/MPAS na grade regular lat-lon. Os campos representados
# são médias diárias (mm/dia).
# ==============================================================

# --- Caminhos dos arquivos NetCDF ---
path_gpm_remap = "gpm_remap_to_mpas.nc"
...
ds_gpm = xr.open_dataset(path_gpm_remap)  # GPM remapeado para grade do MPAS
prec_gpm = ds_gpm['precipitation'].mean(dim='time') * 24  # mm/dia

ds_mpas = xr.open_dataset(path_mpas)
prec_mpas = (ds_mpas['rainc'] + ds_mpas['rainnc']).diff('Time').mean(dim='Time') * 8  # mm/dia
...
# --- Cria figura com dois painéis lado a lado ---
fig = plt.figure(figsize=(12, 5), facecolor='black')

# =============================
# Painel 1 – GPM (remapeado)
# =============================
ax1 = plt.subplot(1, 2, 1, projection=ccrs.PlateCarree())
ax1.set_facecolor('black')
cf1 = ax1.pcolormesh(lon, lat, prec_gpm, cmap=cmap_gpm, norm=norm)
ax1.add_feature(cfeature.COASTLINE.with_scale('110m'), edgecolor='white')
ax1.set_title('GPM – IMERG (remapeado)', color='white', fontsize=12)
...
plt.tight_layout()
plt.savefig("comparacao_gpm_remap_mpas_blackstyle.png", dpi=300, facecolor=fig.get_facecolor())

# --- Fecha arquivos NetCDF ---
ds_gpm.close()
ds_mpas.close()
